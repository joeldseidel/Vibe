...
let handleDrawingArtifacts = function(apiReturn){
    if(apiReturn.success){
        artifacts = apiReturn.artifacts;
        redrawCanvas();
    }
};
drawConn.onopen = function(){
    var openArgs = { type : "new-client", cid : cid };
...
function emitNewLine(thisLine, loc){
    ...
    var cmd = {type : "new-path", path : thisLine, room : roomAssignment};
    //Announce that there is a new path
    drawConn.send(JSON.stringify(cmd));
    cmd = {type: "update-draw", point: loc, room : roomAssignment};
...
function handleCommand(e){
    ...
    if(drawCmd.type === "new-path"){
        ...
        me = drawCmd.me;
        getDrawingArtifacts(cid, handleDrawingArtifacts);
...
function isRelevantCommand(cmd){
    if(cmd.type === "welcome"){
        return true;
    }
    if(roomAssignment === -1) {
        roomAssignment = cmd.room;
        console.log(roomAssignment);
        return true;
    }
    return roomAssignment === cmd.room;
}
